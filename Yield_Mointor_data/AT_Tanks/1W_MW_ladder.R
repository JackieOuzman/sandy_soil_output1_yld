...
harvest_data_file_path <-  readxl::read_excel(
  paste0(metadata_path,metadata_file_name),
  sheet = "file location etc") %>% 
  filter(Site == site_number) %>% 
  filter(variable == "yield files projected thinned") %>% 
  pull("file path")
...
yld_data_with_ladders_clus <- yld_data_with_ladders
names(yld_data_with_ladders_clus)
## No zone and no clm called treat
## create a new clm that is called treat
unique(yld_data_with_ladders_clus$treat_desc)
unique(yld_data_with_ladders_clus$treat)
...
yld_data_with_summary <- yld_data_with_ladders_clus %>% 
  group_by(treat_Ladder_PointID, treat, 
           #treat_id, 
           treat_desc, Ladder_PointID) %>% 
...
ggplot() +
  geom_sf(data = yld_data_with_summary_pt %>% filter(!is.na(treat)), color = "red", size = 1) +
  geom_sf(data = ladders_qgis, fill = NA, color = "black", linewidth = 0.5) +
  #geom_sf(data = zones, fill = NA, color = "blue", linewidth = 0.5) +
  coord_sf(xlim = st_bbox(ladders_qgis)[c(1, 3)],
           ylim = st_bbox(ladders_qgis)[c(2, 4)]) +
  theme_minimal()
