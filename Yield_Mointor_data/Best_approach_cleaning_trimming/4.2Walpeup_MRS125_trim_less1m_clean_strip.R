...
subfolder2 <- "3.clean_trim_per_strip"
...
### remove values zero or below

harvest_filtered <- harvest_filtered %>%  
  filter(target.variable > 0 & !is.na(target.variable))

###############################################################################

harvest_trimmed <- harvest_filtered %>%
  group_by(treat) %>%
  mutate(
