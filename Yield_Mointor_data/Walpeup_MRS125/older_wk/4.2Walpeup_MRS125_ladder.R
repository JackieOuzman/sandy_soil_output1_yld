...
harvest_raw_ish <- st_read(
  paste0(
    ...
    "Yld_WalpeupMRS125_GDA2020_MGA54_thinned.shp")
  )

str(harvest_raw_ish)
#############
## work out which clm to use ### Stirling doesn't want to adjust for moisture
### Christina made a new clm called YldDry12 using her tools
harvest_raw_ish
names(harvest_raw_ish)

harvest_raw_ish %>%
  select(VRYIELDMAS, WetMass, Moisture, DRYMATTER, YldDry12) %>% 
  summary()
...
###############################################################################
#Load in the zones
zones <-st_read(paste0(
  headDir,zone_shapefile_path))
  

################################################################################
### Load in the ladders that I made in qgis using Christina tools

ladders_qgis <-st_read(paste0(
    headDir,
    "/8.Yield_Data/25/Processed/",
    "MRS125_SegmenttsFinalMGA2020_Dist10.0_VarWidth_poly.shp"))
...
ggplot() +
  geom_sf(data = harvest_raw_ish, color = "red", size = 1) +
