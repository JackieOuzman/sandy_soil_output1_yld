...
knitr::include_graphics(paste0(site_name, "_images/Zone map Jackie.png"))

# knitr::include_graphics(c(
#   paste0(headDir, "/3.Covariates/6.Clusters_Zones/FINAL/Zone map Jackie.png")
# ))
```
...
zones_table <- zones %>%
  ...
  mutate(
    Zone = as.character(cluster), #gridcode
...
1. Data projected and thinned to 1m spacing
2. No cleaning or trimming; yield not adjusted for moisture
3. Outliers removed by visual inspection
4. Data clipped to trial area boundaries
5. Polygon ladders generated (10m length × strip width)
6. Yield points within each polygon averaged
7. Averaged values used for statistical analysis
...
knitr::include_graphics(
  ...
  paste0(site_name, "_images/plots/", "yld data with 2 harvesters", ".png"))
)

# ## NB Results are here   
# ## "\\fs1-cbr.nexus.csiro.au\{af-sandysoils-ii}\work\Output-1\1.Walpeup_MRS125\10.Analysis\2#5\###Processing_Jackie"
# ## NB Analysis script is here
