---
title: "Paddock Report: Mervs West - Wynarka"
...
#zone_soil_grid_summary <- zone_soil_grid_summary %>% dplyr::rename(cluster = gridcode )
#zone_soil_grid_summary <- zone_soil_grid_summary %>% dplyr::rename(cluster = cluster3  )
zone_soil_grid_summary <- zone_soil_grid_summary %>% dplyr::rename(cluster = fcl_mdl  )
...
# This is for MRS125 - some odd values that need to be checked
# df_no_cm_long <- df_no_cm %>%
#   select(-zone_short_name, -Id) %>%
#   pivot_longer(cols = -cluster, names_to = "variable", values_to = "value") %>%
#   pivot_wider(names_from = cluster, values_from = value) %>%
#   mutate(across(where(is.character), \(x) ifelse(grepl("e\\+[0-9]{2,}", x), "—", x))) %>%
#   mutate(across(where(is.numeric), \(x) round(x, 2)))  
#   
...
# Note at this site there was no strips with zone 3 Mervs
...
Note the control strip at Mervs West was moved in 2025.

No point sampling was taken within the new control in 2025. 

The below point sampling analysis is based on original control strip.
