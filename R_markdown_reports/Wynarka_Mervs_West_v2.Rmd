---
title: "Paddock Report: Mervs — Wynarka"
...
knitr::kable(rain_table2 %>% 
               mutate(
                 `GS Decile` = stringr::str_to_title(gsub("_", " ", `GS Decile`)),
                 `Summer Decile` = stringr::str_to_title(gsub("_", " ", `Summer Decile`)),
                 `GS Rainfall (mm)` = round(`GS Rainfall (mm)`, 0),
                 `Summer Rainfall (mm)` = round(`Summer Rainfall (mm)`, 0)
               ) %>%
...
strip_dims <- strip %>%
  ...
  summarise(
    Length_m = round(mean(Length_m), 0),
    Width_m = round(mean(Width_m), 0)
  )
...
caption <- paste0("Average strip dimensions: ", strip_length, " m × ", strip_width, " m")
...
knitr::kable(summary_table %>% mutate(across(everything(), ~replace_na(as.character(.), ""))),
             caption = caption,
             col.names = c('Treatment', 'Zone 1', 'Zone 2', 'Zone 3','Zone 3','Total (ha)'),
             booktabs = TRUE)
...
Note the control strip was moved in 2025, no point sampling was taken within the new control.
