...
sowing_table <- Sowing_metadata %>%
  dplyr::select(Year, Crop, Variety, `Sowing date`, `Harvest date`, 
...
Please confirm what layers were used to create zones.
No images on shared drive.
...
#   paste0(headDir, "/3.Covariates/7.pngs/Gums_0-10_pH_Veris_uncalibrated_v2.png"),
#   paste0(headDir, "/3.Covariates/7.pngs/Gums_elevation.png"),
#   paste0(headDir, "/3.Covariates/7.pngs/Gums_em.png"),
#   paste0(headDir, "/3.Covariates/7.pngs/Gums_gamma.png"),
#   paste0(headDir, "/3.Covariates/7.pngs/Gums_satellite.png"),
#   paste0(headDir, "/3.Covariates/7.pngs/Gums_sentinel_ndvi.png")
...
# knitr::include_graphics(c(
#   paste0(headDir, "/3.Covariates/6.Clusters_Zones/FINAL/zones jackie.png")
# ))
```
...
zones_table <- zones %>%
  ...
  dplyr::select(Zone, Area_ha, Percent) %>%
  bind_rows(
    data.frame(
      Zone = "Total",
      Area_ha = sum(.$Area_ha),
...
#"H:\Output-1\4.Wharminda\5.Trial_Plan\FINAL-Trial-Plan\GIS\strips jackie.png"
```
...
strip <- strip %>%
  ...
  ungroup() %>%
  dplyr::select(-bb, -x_dist, -y_dist)

strip_dims <- strip %>%
  st_drop_geometry() %>%
  dplyr::filter(treat_desc != "Buffer") %>%
  dplyr::select(Length_m, Width_m) %>%
...
joined <- joined %>% dplyr::select(-POLY_AREA, -Area_ha)
...
summary_table <- joined %>%
  ...
  #dplyr::rename(Zone = fcl_mdl) %>%
  #dplyr::rename(Zone = cluster3) %>%
  dplyr::rename(Zone = cluster) %>%
  st_drop_geometry() %>%
  #filter(treat_desc != "Outside Control" & treat_desc != "Buffer") %>%
  dplyr::select(treat_desc, Zone, Area_ha) %>%
  dplyr::mutate(Area_ha = round(Area_ha, 2)) %>%
  pivot_wider(names_from = Zone,
              values_from = Area_ha,
              names_prefix = "Zone ") %>%
  dplyr::mutate(Total = round(rowSums(across(starts_with("Zone")), na.rm = TRUE), 2))
...
## Note at this site there was no strips with zone 3. This is for CB HO
...
Point_sampling <- readxl::read_excel(
  ...
  dplyr::select("Site", "analysis_type", "Date_collected") %>%
  dplyr::mutate(Date_collected = as.Date(as.numeric(Date_collected), origin = "1899-12-30"))

#names(Point_sampling)
point_table <- Point_sampling %>%
  dplyr::select(analysis_type, Date_collected)
```
...
#"H:\Output-1\4.Wharminda\7.In_Season_data\25\sampling locations jackie.png"
```
...
Note:
At Gum; only a few maturity results. 
